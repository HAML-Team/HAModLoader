...
public class NewMenuController : MonoBehaviour
{
    ...
    public static System.Action OnRequestModMenuOpen;
    public static System.Action OnRequestModMenuClose;
    public static System.Func<bool> IsModMenuOpenCheck;
    ...
    public void OpenModMenu()
    {
        ...
        PopupControl.Instance.black_background.SetActive(true);
        submenu.SetActive(true);
        submenu_header.text = "MOD CONTROL";
        OnRequestModMenuOpen?.Invoke();
    }

    public void Cancel()
    {
        in_sumenu = false;
        bool modMenuIsOpen = IsModMenuOpenCheck?.Invoke() ?? false;
        if (modMenuIsOpen)
        {
            OnRequestModMenuClose?.Invoke();
